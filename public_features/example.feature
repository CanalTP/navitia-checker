...
Scenario: Calcul d'itinéraire accessible (ligne à 1 et 2 arrêts à 5)
    ...
    Then on doit me proposer la suite de sections suivante : "GARE D'AULNAY SOUS BOIS (Aulnay-sous-Bois) ==[ Tramway T4 - TRAMWAY ]==> GARE DE BONDY (Bondy) "
...
Scenario: Volumétrie des données carto
    When je demande des infos sur les données carto
    Then on doit m'indiquer un total de "110" éléments pour les "régions administratives hors périmètre" avec une tolérance de "5"
